use std::{array::from_fn, marker::PhantomData, mem, sync::Arc};
...
use crate::{
    ...
    gkr::{
        GKRCircuit, GKRCircuitWitness, GKRProof, GKRProverOutput,
...
fn keccakf_1(state: &[bool]) -> Vec<bool> {
    assert_eq!(state.len(), STATE_SIZE);

    let state64 = bools_to_u64s(state);
    let mut state64: [u64; 25] = from_fn(|i| state64[i]);

    keccakf(&mut state64);

    u64s_to_bools(&state64)
}
        ...
        (0..ROUNDS)
            ...
            .fold(final_output, |acc, round| {
                ...
                chip.add_layer(Layer::new(
                    format!("compute theta-rho-pi-chi-iota output {round}"),
                    LayerType::Zerocheck,
                    exprs,
                ...
                let exprs = (0..STATE_SIZE)
                    .map(|i| chi_expr(i, &pi_output.iter().map(|e| e.0).collect_vec()))
                    .collect_vec();
                ...
                chip.add_layer(Layer::new(
                    ...
                    // vec![challenge1, challenge2],
                    vec![],
                    pi_output.iter().map(|e| e.1.clone()).collect_vec(),
                    vec![],
                    chi_output.iter().map(|e| e.1.clone()).collect_vec(),
                ));
                ...
                // The explicit bijection that applies the effects of phases rho and pi combined
                let perm = rho_and_pi_permutation();
                let exprs = (0..STATE_SIZE)
                    .map(|e| theta_output[perm[e]].0.into())
                    .collect_vec();

                chip.add_layer(Layer::new(
                    "compute theta-rho-pi output".to_string(),
                    LayerType::Zerocheck,
                    exprs,
                    // vec![challenge1, challenge2],
                    vec![],
                ...
                let c_wits = c.clone().into_iter().map(|e| e.0).collect_vec();
    ...
    E: ExtensionField,
{
    ...
    fn gkr_witness(&self, phase1: &[Vec<E::BaseField>], challenges: &[E]) -> GKRCircuitWitness<E> {
        ...
        for i in 0..24 {
            ...
            layer_wits.push(LayerWitness::new(
                pi(&rho(&theta(bits.clone()).try_into().unwrap()))
                    .into_iter()
                    .map(|b| vec![b])
                    .collect_vec(),
                vec![],
            ...
            layer_wits.push(LayerWitness::new(
                chi(&pi(&rho(&theta(bits.clone()).try_into().unwrap())).to_vec())
                    .into_iter()
                    .map(|b| vec![b])
                    .collect_vec(),
                vec![],
            ...
            if i < 23 {
                ...
                dbg_vec(&value);
                layer_wits.push(LayerWitness::new(
                    value.clone().into_iter().map(|b| vec![b]).collect_vec(),
                    vec![],
                ));
...
fn rho<T: Copy + Default>(state: &[T; STATE_SIZE]) -> [T; STATE_SIZE] {
    let (mut x, mut y) = (1, 0);
...
fn pi<T: Copy + Default>(state: &[T; STATE_SIZE]) -> [T; STATE_SIZE] {
    let mut ret = [T::default(); STATE_SIZE];
...
fn rho_and_pi_permutation() -> [usize; STATE_SIZE] {
    ...
    let out_evals = {
        ...
        if test {
            ...
            let state = u64s_to_bools(&state)
                .into_iter()
                .map(|b| Goldilocks::from(b as u64))
                .collect_vec();
